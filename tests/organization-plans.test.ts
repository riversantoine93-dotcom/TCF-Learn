...
describe("organization plans",()=>{
  ...
  it("uses the organization bundle prices",()=>{
    expect(ORGANIZATION_PLANS["org-10"].amountCents).toBe(97000);
    expect(ORGANIZATION_PLANS["org-50"].amountCents).toBe(388000);
    expect(ORGANIZATION_PLANS["org-100"].amountCents).toBe(582000);
  });
});
