...
export default function Course(){
 ...
 return <main><Header/><section className="course-hero"><div className="shell"><span className="eyebrow">INTERACTIVE COURSE</span><h1>Turning Forward</h1><p>The Work Beyond Fear</p>{enrolled?<ProgressBar value={Math.round(completedSections/32*100)}/>:<div className="notice">Preview all 8 modules below. Full course access is a one-time $97 purchase through the TCF Learn Course Store.</div>}</div></section><section className="shell module-list">{modules.map(m=>{const open=enrolled&&(m.number===1||moduleIsComplete(p,m.number-1));const count=moduleProgress(p,m.number);return <article className="module-card" key={m.slug}><div className="module-number">{String(m.number).padStart(2,"0")}</div><div><span>{m.lessons} lessons + challenge</span><h2>{m.title}</h2><p>{m.description}</p>{enrolled&&open&&<small>{count} of 4 sections complete</small>}{enrolled&&!open&&<small>🔒 Complete Module {m.number-1} to unlock</small>}</div>{!enrolled?<span className="locked-pill">Preview</span>:open?<Link className="button secondary" href={`/course/turning-forward/${m.slug}`}>{count?"Continue":"Begin"}</Link>:<span className="locked-pill">Locked</span>}</article>})}</section>{!checking&&!enrolled&&<section className="shell" style={{paddingBottom:"64px",textAlign:"center"}}><h2>Ready to enroll?</h2><p>Purchase Turning Forward by itself for $97, or choose the $145.50 two-course bundle in the central TCF Learn Course Store.</p><Link className="button" href="/dashboard#purchase-center">View Course Store & Pricing</Link>{user&&<p><small>You are signed in, but this account does not currently have an active Turning Forward enrollment.</small></p>}</section>}</main>
}
