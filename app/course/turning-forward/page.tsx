...
export default function Course(){
 ...
 useEffect(()=>{if(loading)return;if(!user||!supabase){setEnrolled(false);setChecking(false);return;}setChecking(true);supabase.from("enrollments").select("id").eq("user_id",user.id).eq("course_slug","turning-forward").eq("active",true).maybeSingle().then(({data})=>setEnrolled(Boolean(data))).finally(()=>setChecking(false));},[user,loading]);
