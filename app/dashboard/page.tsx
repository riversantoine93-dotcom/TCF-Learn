...
export default function Dashboard(){
  ...
  return <main className={`dashboard-shell ${theme}`}>
    ...
    <section className="dashboard-main">
      ...
      <div className="hub-heading" id="my-courses"><div><span className="eyebrow">TCF LEARN COURSE HUB</span><h2>My Courses</h2><p>Purchased courses and progress live here. New purchases are made in the Course Store above.</p></div></div>
      <div className="course-hub-grid">
        <article className="hub-course-card available"><div className="course-thumb"><Image src="/course-thumbnails/turning-forward.png" alt="Turning Forward course thumbnail" fill sizes="(max-width: 900px) 100vw, 50vw" priority/><span className="status-pill">COURSE</span></div><div className="course-card-body"><h3>Turning Forward: The Work Beyond Fear</h3><p>A structured path from survival mode to clarity, discipline, purpose, and forward momentum.</p><div className="progress-heading"><b>Overall Progress</b><strong>{turningPercent}%</strong></div><ProgressBar value={turningPercent}/><div className="course-meta"><span>{turningComplete} of 32 sections complete</span><span>8 modules</span></div><Link className="continue-button" href="/course/turning-forward">{turningComplete?"Continue Course":"View Course"}<span>→</span></Link></div></article>
        <article className="hub-course-card available"><div className="course-thumb"><Image src="/course-thumbnails/from-thought-to-freedom.png" alt="Thought to Freedom course thumbnail" fill sizes="(max-width: 900px) 100vw, 50vw"/><span className="status-pill">{thoughtComplete?"COMPLETED":thoughtStarted?"IN PROGRESS":"COURSE"}</span></div><div className="course-card-body"><h3>Thought to Freedom</h3><h4>Correcting Criminal Thinking Errors</h4><p>Identify recurring thinking errors, challenge the thought before it becomes action, and practice responsible replacement thinking.</p><div className="progress-heading"><b>Core Lesson Progress</b><strong>{thoughtSummary.percent}%</strong></div><ProgressBar value={thoughtSummary.percent}/><div className="course-meta"><span>{thoughtSummary.complete} of 30 lessons complete</span><span>10 modules</span></div><Link className="continue-button" href="/course/thought-to-freedom">{thoughtComplete?"Review Course":thoughtStarted?"Continue Course":"View Course"}<span>→</span></Link></div></article>
      </div>
      <div className="next-step"><div><span>→</span><div><b>Your next step</b><p>{thoughtStarted&&!thoughtComplete?"Continue Thought to Freedom from your next incomplete thinking-error lesson.":turningComplete?"Continue the course that matters most to your current work.":"Choose a course in the store above or preview either course before purchasing."}</p></div></div><Link href={thoughtStarted?"/course/thought-to-freedom":"/dashboard#purchase-center"}>{thoughtStarted?"Open Course":"View Course Store"}</Link></div>
    </section>
