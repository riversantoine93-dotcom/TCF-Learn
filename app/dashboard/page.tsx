  ...
  return <main className="dashboard-shell">
    ...
    <section className="dashboard-main">
      ...
      <div className="course-hub-grid">
        {courseCatalog.map(course=><article className={`hub-course-card ${course.status}`} key={course.slug}>
          <div className="course-thumb"><Image src={course.image} alt={`${course.title} course thumbnail`} fill sizes="(max-width: 900px) 100vw, 50vw" priority={course.slug==="turning-forward"}/><span className="status-pill">{course.status==="available"?"ENROLLED":"COMING SOON"}</span></div>
          <div className="course-card-body"><h3>{course.title}</h3>{course.subtitle&&<h4>{course.subtitle}</h4>}<p>{course.description}</p>
          {course.status==="available"?<><div className="progress-heading"><b>Overall Progress</b><strong>{percent}%</strong></div><ProgressBar value={percent}/><div className="course-meta"><span>{complete} of 32 sections complete</span><span>8 modules</span></div><Link className="continue-button" href={course.href}>{complete?"Continue Course":"Start Course"}<span>→</span></Link></>:<div className="coming-state"><span className="lock-icon">▣</span><div><b>COMING SOON — {course.release}</b><small>Enrollment details will be announced closer to launch.</small></div></div>}
          </div>
        </article>)}
      </div>
