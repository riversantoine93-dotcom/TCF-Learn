...
export default function Dashboard(){
  ...
  const nextCourse=hasThoughtToFreedom&&thoughtStarted&&!thoughtComplete?"/course/thought-to-freedom":hasTurningForward?"/course/turning-forward":hasThoughtToFreedom?"/course/thought-to-freedom":"/organizations";
  return <main className={`dashboard-shell ${theme}`}>
    <aside className="dashboard-sidebar"><Link className="dash-brand" href="/dashboard"><strong>TCF <span>LEARN</span></strong><small>TURNING CONVICTION INTO FREEDOM</small></Link><nav aria-label="User navigation"><Link className="active" href="/dashboard"><span>⌂</span>Dashboard</Link><Link href="/courses"><span>▤</span>Courses</Link><Link href="/organizations"><span>◇</span>Organization Plans</Link><Link href="/dashboard#my-courses"><span>▣</span>My Courses</Link><Link href="/profile"><span>○</span>Profile</Link></nav><button type="button" className="theme-toggle" onClick={toggleTheme} aria-label={`Switch to ${theme==="dark"?"light":"dark"} mode`}><span>{theme==="dark"?"☀":"☾"}</span><b>{theme==="dark"?"Light Mode":"Dark Mode"}</b></button></aside>
    <section className="dashboard-main">
      ...
      {user&&<div className="dash-stats"><div><strong>{checkingEnrollments?"—":enrolledCourses.length}</strong><span>Active Courses</span></div>{hasTurningForward&&<div><strong>{turningPercent}%</strong><span>Turning Forward</span></div>}{hasThoughtToFreedom&&<div><strong>{thoughtSummary.percent}%</strong><span>Thought to Freedom</span></div>}</div>}
      <div className="hub-heading" id="my-courses"><div><span className="eyebrow">TCF LEARN COURSE HUB</span><h2>My Courses</h2><p>Courses assigned to this account through an active TCF Learn organization license appear here.</p></div></div>
      {checkingEnrollments?<div className="course-empty-state"><h3>Checking your course access…</h3><p>We’re loading the active courses attached to your account.</p></div>:enrolledCourses.length===0?<div className="course-empty-state"><h3>No organization course access yet</h3><p>Your organization administrator can assign you a learner seat. Every learner seat includes both TCF Learn courses.</p><Link className="continue-button" href="/organizations">View Organization Plans <span>→</span></Link></div>:<div className="course-hub-grid">
      ...
      <div className="next-step"><div><span>→</span><div><b>Your next step</b><p>{enrolledCourses.length===0?"Your organization administrator can assign your learner seat to begin.":hasThoughtToFreedom&&thoughtStarted&&!thoughtComplete?"Continue Thought to Freedom from your next incomplete thinking-error lesson.":"Continue one of the courses attached to your account."}</p></div></div><Link href={nextCourse}>{enrolledCourses.length===0?"Organization Plans":"Open Course"}</Link></div>
    </section>
