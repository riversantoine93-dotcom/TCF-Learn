...
export default function GuidedStoryLesson({story}:{story:GuidedStory}){
  ...
  return <section className={styles.story} aria-label="Guided lesson">
    ...
    <details className={styles.keyNotes}>
      <summary><span><b>KEY NOTES</b><small>Terms &amp; ideas from this lesson</small></span><span className={styles.keyNotesAction}>OPEN +</span></summary>
      <div className={styles.keyNotesBody}>{story.teachingCards.map((card,index)=><article className={styles.keyNoteItem} key={card.title}><span>{String(index+1).padStart(2,"0")}</span><div><h3>{card.title}</h3><p>{card.body}</p><small><b>Why it matters:</b> Use this idea to recognize the lesson in real situations and choose a more intentional response.</small></div></article>)}</div>
    </details>
